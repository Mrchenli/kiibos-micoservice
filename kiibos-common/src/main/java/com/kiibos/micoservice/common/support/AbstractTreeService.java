package com.kiibos.micoservice.common.support;



import com.google.common.collect.Lists;
import com.kiibos.micoservice.common.util.PublicUtil;
...
public abstract class AbstractTreeService<T extends BaseTree<T, ID>, ID extends Serializable> implements ITree<T, ID> {
	...
	public List<T> getChildTreeObjects(List<T> list, ID parentId) {
		...
		for (T res : list) {//遍历 每个获取它的children就可以了
			if (res.getPid() == null) {
				continue;
			}
			if (Objects.equals(res.getPid(), parentId)) {
				recursionFn(list, res);//把第一层的往下封装好
				returnList.add(res);
			}
		}
		return returnList;
